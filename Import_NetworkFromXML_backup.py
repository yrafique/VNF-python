...
def read_links(linklist):
    ...
    for i, link in enumerate(linklist):
        ...
        destination = link.getElementsByTagName("target")[0].firstChild.data  # Changed to 'target' based on your XML
        additionalModules = link.getElementsByTagName("additionalModules")
        if additionalModules:
            addModule = additionalModules[0].getElementsByTagName("addModule")
            if addModule:
                capacity = float(addModule[0].getElementsByTagName("capacity")[0].firstChild.data)
                cost = float(addModule[0].getElementsByTagName("cost")[0].firstChild.data)
                links[link_id] = {'source': source, 'destination': destination, 'additionalModules': {'addModule': {'capacity': capacity, 'cost': cost}}}
        else:
            # Assuming you still want to add the link even if it has no 'additionalModules'
            links[link_id] = {'source': source, 'destination': destination}
    return links
